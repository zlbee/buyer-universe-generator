...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    @app.get("/buyers/acquirer-capable-universe", tags=["buyers"])
    def build_acquirer_capable_universe(request: Request, query: str = Query(min_length=1)) -> dict:
        session_factory = create_session_factory(request.app.state.engine)
        with session_factory() as session:
            service = build_acquirer_capability_universe_builder(active_settings, session)
            try:
                result = service.build_universe(query)
            except AmbiguousTargetError as error:
                raise HTTPException(
                    status_code=409,
                    detail={
